...
setTimeout(function() {
    ...
    window.interval2 = setInterval(function() {
        if(window.flag) {
            currentpos = $("div[widgetname=REPORT0_C]A").find(".reportContent")[0]A.scrollTop;
            if(currentpos == old) {
                $("div[widgetname=REPORT0_C]A").find(".reportContent")[0]A.scrollTop = 0;
            } else {
                old = currentpos;
                $("div[widgetname=REPORT0_C]A").find(".reportContent")[0]A.scrollTop = currentpos + 1.5;
            }
        }
    }, 25);
}, 1000)]]></Content>
...
<TemplateID TemplateID="249c0fe6-ea2a-4532-8d1b-df196d148a96"/>
